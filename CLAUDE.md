...
**The flag does not reach the workflows for free.** `telegram_configured()` covers every *application* path, but three workflows also POST to the Telegram API with **raw curl** in a `if: failure()` step — `daily_harvest.yml`, `riskguard_premarket.yml` and `db_backup.yml` — so each repeats the check itself. That was missed when the switch landed (2026-08-12): setting the flag false silenced everything *except* the "🔴 FAILED" alerts, which is precisely the case the flag gets set for. `tests/test_workflow_telegram_guard.py` now pins it, scanning the YAML and failing on any Telegram-calling step that doesn't honour the flag; it pins the exact *set* of such steps too, because `db_backup.yml` was missed twice — it alerts on the same channel but isn't one of the two "alerting workflows". **Consequence, and it is the accepted cost of switching off:** `TELEGRAM_TOKEN` is deliberately unset on the Zeabur `cron` service, so Actions is the only path that alerts on failure — with the flag false, **a failed harvest, Risk Guard run or DB backup is silent everywhere**, and the Actions run log is the only record. Each guarded step echoes why it stayed quiet so the log still says what happened.
...
### The operator console (`/d/<token>/`)

Every web surface hangs off one prefix behind one nav frame: `/d/<token>/` overview, `/system` (generated from the live tool registry), `/flow` (the built dashboard), `/graph`, `/tickers`, `/t/<ticker>`. Before 2026-08-11 these were five unrelated documents at three prefixes with no links between them, so using any of them meant already knowing its URL. `/g/`, `/t/` and `/h/` still resolve for old links — don't add anything new there.

`console.py` owns the shell: `NAV` is the single source of IA, `shell()` renders pages the console owns, and `inject_nav()` retrofits the same bar onto documents `src.dashboard.*` pre-rendered. **Every nav link is relative** — that is what lets a static file generated by the harvester, which never sees the bearer token, still link correctly once served under the token prefix. Don't make them absolute.
