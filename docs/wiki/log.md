...
## [2026-08-12] fix+feat | Overview was 500ing in production (bad mock hid it); self-explaining Market page added
attributed_to: [niko, claude-agent]   belongs_to: [mcp-server, risk-guard]
[niko] asked for a comprehensive dashboard that "explains the numbers and functions by itself". Found and fixed a **live 500 first**: `console_pages.overview_html` iterated `db_v2.query_data_status()` as a list of row dicts, but that function returns a single dict `{table_counts, latest_t86_date, recent_ingestions}` — so `/d/<token>/`, the console's entry point shipped in PR #2, raised `AttributeError: 'str' object has no attribute 'get'` on **every request**. **The test suite did not catch it because the test mocked the function with the author's assumed shape** — the mock encoded the mistake rather than checking for it, and passed. Lesson recorded in `tests/test_console_pages.py`: fixtures now mirror the shape the real function constructs, with a docstring naming where it comes from; the old shape-guessing tests were deleted rather than adjusted, since a test that can pass while the page is broken is worse than no test. **New self-explaining Market page** (`/d/<token>/market`): today's risk light, then each of the five checks rendered with (1) what it measures in plain words — "margin balance is stock bought with borrowed money; rising leverage into a falling market is retail that hasn't given up", (2) the exact rule it is judged by, **interpolated from `rg.config` rather than retyped**, so the page can never disagree with the scorer that produced the number, (3) the inputs it actually saw, (4) why it did or did not add to the score. A `data_missing` check explains that it is reported absent rather than scored zero *because* a neutral substitute reads as "calm", and calm is the dangerous default for a warning system. **Overview rewritten to explain itself too**: each table gets a plain-language description ("who bought and sold each stock" beats `raw_twse_t86`), freshness rendered as words rather than a date the reader must interpret ("yesterday's session", "3 days ago — normal across a weekend", "22 days ago — the harvest has stalled"), and the last few harvest runs with their result. 15 new tests; suite 519 pass, focused ruff clean.

## [2026-08-12] ops+security | Migration moved to a manual Actions workflow; second production credential exposed
attributed_to: [niko, claude-agent]   belongs_to: [paid-connector-deploy, infrastructure-accounts]
[niko] pasted the **Zeabur owner DSN including its password** into a chat so the blocking migration could be run. I could not use it: this agent container's only egress is an HTTP proxy which accepts `CONNECT` to arbitrary host:port but does not pass non-TLS bytes, so a raw Postgres handshake to `8.209.197.81:32046` establishes a tunnel and then times out. Verified twice, with and without the documented `gssencmode=disable` fix. Local credential copies were shredded and the bridge process killed. **This is the second credential exposure in this project** (the first: a Neon owner DSN echoed to a terminal by `apply_schema.py` on 2026-08-09), and both had the same root cause — a migration that requires a production owner credential to travel to whatever machine is doing the work. **Fix: `.github/workflows/db_migrate.yml`** — `workflow_dispatch` only, gated on typing `apply`, running `apply_delta.py --yes` with the `DATABASE_URL` secret that Actions **already holds and already uses daily** for the harvesters. The credential now stays where it lives; nobody needs to paste it anywhere. `apply_delta.py` was already safe for this — it prints only the target host, never the DSN, and reads privileges back afterwards, failing the run if `mcp_viewer` still cannot SELECT. **Action required: rotate the Zeabur `root` password**, since the exposed one is now in a chat transcript. Runbook and infrastructure page updated with the new path.

## [2026-08-12] ops | Migrations 019–025 APPLIED to production; grant bug confirmed fixed live
attributed_to: [niko, claude-agent]   belongs_to: [paid-connector-deploy, system-architecture]
The blocking migration is **done**. [niko] dispatched `DB Migrate (manual)` (run #1, 18s, `workflow_dispatch` on `main@a386909`) and it succeeded, ending with the line that actually matters: **`mcp_viewer SELECT verified on 7 backfilled tables` → `Delta applied ✓`**. That is the privilege *read-back*, not just "the SQL ran" — the bug being repaired was a grant that executed, reported nothing and did nothing, so a run without that assertion would have proved nothing. **Confirmed live through the connector immediately after:** `session_state` now returns `calendar_source: "calendar"` where it returned `"weekend_only"` for weeks — `market_holidays` is one of the seven backfilled tables, so this is direct evidence the grants took effect on the running server, not merely in the database. The remaining verifications (`q_valuation`, `dividend_calendar`, `set_my_risk_profile` persisting via the new `owner` row from 025) were gated behind the connector's tool-approval prompt at the time of writing; [codex-agent] ran them on 2026-08-16 and all passed — see the entry below. Two things still need a scheduled run before they can change: the M1 margin subitem (needs one post-close pass now that the lag bound exists) and Telegram `pushed:true` (needs one run holding a valid token). **Open and urgent: the Zeabur `root` password was pasted into a chat transcript and must be rotated**, then updated in the `DATABASE_URL` repo secret and the `mcp` service's `MCP_DATABASE_URL`. The only reason the paste happened is that this agent container cannot reach the database — which `db_migrate.yml` now makes permanently unnecessary.

## [2026-08-12] feat | TELEGRAM_ENABLED kill switch — one flag silences every outbound message
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, risk-guard]
[niko] asked to disable Telegram. Implemented as an explicit flag rather than by removing the token, and that distinction is the whole design: **an unset token and a deliberate "off" look identical at the send site, and they are not the same thing.** An unset token in a system that believes it is alerting is a breakage — it is precisely how every Risk Guard alert sat at `pushed:false` for weeks (2026-08-10) while the notify-on-failure path, depending on the same token, was itself silently dead. So `TELEGRAM_ENABLED=false` (accepting `false/0/no/off`, default on) now gates `src/config.telegram_configured()`, which every outbound path already funnels through — Risk Guard stop alerts, the post-close brief, the daily summary, the thesis heartbeat, and the news poller's watchlist pushes. One flag, no send site changed. `send()` logs the two cases **differently on purpose**: switched-off at INFO (routine), missing token still at WARNING (a system lying to itself); a test pins that, because warning-level noise on every scheduled run is what trains an operator to ignore the channel that reports real breakage. `bot.py` re-declares the flag independently — it ships in the mcp_server image and cannot import from the repo root (deployment split) — so the same variable also stops the bot answering commands on that service, while leaving the command surface available if only alerts are silenced. Both alerting workflows read `vars.TELEGRAM_ENABLED` and their `getMe` preflight now **skips cleanly when off** instead of warning about something switched off on purpose. Verified end to end: with the flag false and a valid token present, `send()` and `send_error_alert()` make zero network calls. 8 new tests; suite 527 pass, focused ruff clean. Note the test helper had to keep the env patch active through its assertions — `telegram_enabled()` reads `os.getenv` per call while the token constants are captured at import, so a reload-then-restore helper tests the restored value.

## [2026-08-12] chore | Lint debt paid off (185 → 0); CI now enforces `ruff check .` repo-wide
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture]
CI failing on PR #6 exposed a design flaw in the CI I had just written: it lints **whole changed files**, so touching any file carrying pre-existing debt inherits that debt and blocks unrelated work. [niko] asked whether to scope lint to changed *lines* or find something better. **Measured before answering, and the numbers changed the answer:** 185 errors, but 110 auto-fixable and a single rule (`UP045`, `Optional[X]` → `X | None`) was 56% of the total. Not 185 hard problems — one mechanical sweep plus ~33 judgement calls. **Rejected line-scoping** because ruff has no native support (you maintain a diagnostic-to-line-range wrapper forever), and it is wrong in both directions: add an import and `I001` fires on the import block, not your line, so a problem your change caused gets skipped while someone else's shows up in your diff. It also institutionalises the debt instead of removing it. A baseline file has the same flaw plus rot. **The decisive argument was the mirrors:** `src/quant/factor_alpha.py` and `mcp_server/api/quant/factor_alpha.py` carried the same 17 errors each (also `quality_score` 9/9, `pca_decompose` 5/5). Under "fix it when you touch it", someone fixes one side and the mirrors drift — the exact failure CLAUDE.md warns about and nothing detects. One sweep fixes both sides identically in one commit. Also noted the repo **conflated `ruff-format` with `ruff check`**: the standing objection ("would reformat 66 of 84 files") is about formatting and does not transfer to 110 targeted lint fixes. Done in three commits — safe autofixes (110, incl. 13 `F401` imports orphaned by the `UP045` rewrite, invisible until it ran); `B905` (27) with **`strict=True` not ruff's behaviour-preserving `strict=False`**, since every site pairs sequences equal-length by construction and silent truncation in `factor_alpha` means confidently wrong alpha rather than a visible failure — flagged as the one commit that can change runtime behaviour; then `E741`/`E701`/`E702` (46), split at ruff's own reported statement boundaries so semicolons inside string literals were never touched. `ci.yml` is now `ruff check .` with no special-casing; `.git-blame-ignore-revs` added so the mechanical commits don't bury authorship. Suite 527 pass throughout, mirrors verified at one diff line each after every commit, every deployed module still imports.

## [2026-08-13] feat | `momentum_leaders_scan` — the momentum sibling, built stop-first
attributed_to: [niko, claude-agent]   belongs_to: [mcp-server, taiwan-ai-supply-chain]
[niko] supplied a spec for the momentum/growth counterpart to `flow_leaders_scan` while Codex finishes the deploy. Built as tool 49: pure scorer in `mcp_server/api/momentum_leaders.py`, one SQL pass in `db_v2.query_momentum_leaders`, tool + `sc_capabilities` entry in `index.py` (the two-edit rule held — `test_capabilities` confirms 49 registered = 49 advertised, no drift). **All six spec acceptance cases pass**, including the crux: 川湖 2059 is `momentum-entry` in June and `chase` in August on the parabolic guard alone — same stock, opposite verdict. Worth noting it still *scores* highly in August (relative strength and breakout quality are excellent at a top), which is exactly why the anti-flags are evaluated independently of the score rather than folded into it. **Found a real bug while writing the tests**: `trailing_stop` discarded any candidate stop above the current price — correct for entry (you cannot open a position with a stop above your fill) but exactly backwards for `mode="monitor"`, where price falling through the swing low *is* the stop being hit. As written, the monitor would have slid the stop down and reported "hold" forever — the exit signal the mode exists to produce could never fire. Fixed with an explicit `for_entry` flag and three regression tests. **Coverage limit stated rather than papered over:** ATR, the breakout test and the climax guard need high/low/volume, which only `raw_twse_ohlcv` carries (a top-500 harvest), and the 200-day mean needs 200 sessions — so the momentum universe is materially narrower than the sleeper scan's ~1.2k priced names, and the tool returns a `universe_note` saying so. Small caps outside the harvest are not scored at all rather than scored badly; several spec acceptance tickers (晟田 4541, 日馳 1526) may fall outside it live. **Engine separation is tested, not just documented** (§5): high P/E must not move a momentum verdict while it is a sleeper anti-flag; "has not run yet" is a sleeper requirement and a momentum disqualifier; and a structural test asserts the two modules share no constants, since a "shared helper" refactor is how the inversion would silently break. 31 new tests; suite 555 pass, `ruff check .` clean repo-wide.

## [2026-08-16] ops | Post-merge runbook: Zeabur migrations applied, live verified; branch protection/Neon blocked
...
## [2026-08-16] ops+security | Stack #5–#8 merged under branch protection; public repo changes the DB threat model
attributed_to: [niko, claude-agent]   belongs_to: [infrastructure-accounts, system-architecture]
- Merged the four open PRs in dependency order — #5 (wiki record), #6 (`TELEGRAM_ENABLED`), #7 (lint payoff), #8 (`momentum_leaders_scan`). First round actually gated by the new ruleset; `main` is now `ce5ddf3`, suite 558 pass, `ruff check .` clean, 49 tools registered = 49 advertised.
- **GitHub does not auto-retarget a stacked PR when its base merges — only when the base branch is *deleted*.** #7 and #8 kept pointing at already-merged branches and had to be retargeted by hand. A stack left alone looks mergeable while measuring its diff against a dead branch.
- **Strict required checks make a squash-merged stack a treadmill:** each merge moves `main`, so every remaining PR needs a fresh merge-and-rerun. Four PRs cost four CI cycles; that is the price of the gate, not a misconfiguration.
- Every conflict was `docs/wiki/log.md` — two agents appending dated entries at the same point. Both sides are always kept, ordered by date; neither is a competing edit. Resolver: `scratchpad/resolve_log.py`.
- **Security consequence not yet recorded: the repo is public now, and the wiki documents the production Postgres endpoint** — `8.209.197.81:32046`, db `zeabur`, superuser `root`, TLS disabled — across `CLAUDE.md`, `docs/OAUTH-PLAN.md` and four wiki pages. Audited the tree and full history: **no credential is committed** (`.env` is gitignored, the leaked password never entered git history, `git log --all -S` is empty). So the password is the only thing between the internet and that database — and it is already exposed in a chat transcript. **Rotating the Zeabur `root` password moved from "urgent" to "blocking".**

## [2026-08-16] fix | The Telegram kill switch missed three workflows; raw curl never went through the app
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, infrastructure-accounts]
- [niko] set `TELEGRAM_ENABLED=false` on GitHub and Zeabur and restarted the services. Everything routed through `src/config.telegram_configured()` went quiet — but **`daily_harvest.yml`, `riskguard_premarket.yml` and `db_backup.yml` each end with an `if: failure()` step that POSTs to the Telegram API with raw curl**, no Python involved, so the flag never reached them. "Disabled" still sent 🔴 FAILED messages — exactly the case the flag gets set for.
- The claim in `CLAUDE.md` that "one flag silences every outbound message" was true of the application and false of the repo. Corrected, rather than left as a contract the code doesn't honour.
- **`db_backup.yml` was missed twice** — by the original switch and by the first pass of this fix — because it isn't one of the two "alerting workflows" despite using the same channel. It also had no top-level `TELEGRAM_ENABLED`, so the guard had to add one to the step's own `env:`.
- Found by `tests/test_workflow_telegram_guard.py`, written before the third instance was known: it scans the YAML for steps touching `api.telegram.org` and asserts each honours the flag. It pins the exact *set* of such steps, so a new unguarded sender fails rather than joining silently, and it strips comments first — the first version passed on a comment that merely *mentioned* `TELEGRAM_ENABLED`, caught by deleting a real guard and watching it stay green.
- [niko] chose total silence over keeping failure alerts. **Accepted cost:** `TELEGRAM_TOKEN` is deliberately unset on the Zeabur `cron` service, so Actions was the only path alerting on failure — a failed harvest, Risk Guard run or backup is now silent everywhere. Each guarded step echoes why it stayed quiet, so the Actions run log is the remaining record.

## [2026-08-16] ops+decision | Root password rotation declined; an accidental half-rotation found and restored
attributed_to: [niko, claude-agent]   belongs_to: [infrastructure-accounts, system-architecture]
- **[niko] decided not to rotate the exposed Zeabur `root` password** — "no one really cares about this. Just keep it running like normal." Recorded as an accepted risk rather than an open action item. Bounded by a same-day check: `pg_hba_file_rules` shows the catch-all `host all all all` rule is `scram-sha-256`, so the internet-facing endpoint does demand the password and is not open.
- **But the rotation had already half-happened.** `\password root` was run while exploring, which changes the credential immediately; every consumer still held the old one. Caught before any scheduled job hit it (nearest was `news_harvest` at 21:00 Taipei); only the `newswatch` poller, which reconnects every 180s, was actually erroring — and silently, because Telegram had just been switched off and that is an application path.
- Restored with `ALTER USER root PASSWORD '$POSTGRES_PASSWORD'` from inside the container. Chosen because it is **idempotent under both hypotheses**: if the password had rotated it reverts it, and if it had not, it re-sets the same value — so no further diagnosis was needed to act safely.
- **Three traps recorded on [infrastructure-accounts](topics/infrastructure-accounts.md), all costing a wrong turn here:** editing `POSTGRES_PASSWORD` + restart does *not* rotate (the image reads it only at init on an empty data dir); a password cannot be tested over loopback because `initdb` writes `local`/`127.0.0.1`/`::1` as **`trust`** — my first verification command was wrong for exactly this reason and "passed" with an unsubstituted placeholder as the password; and the container's `POSTGRES_PASSWORD` env is the authoritative copy of the original, which makes recovery paste-free.
- Consequence worth keeping: because loopback is `trust`, **superuser access never depends on knowing the password**, so a mis-rotation on this service is always recoverable from the Zeabur Command shell.

## [2026-08-16] security | The dashboard link was the API key; console gets its own token, Cloudflare Access planned
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, mcp-server]
- [niko] asked for a better way to host the dashboard than a token in the URL. Investigating turned up something worse than the URL question: **`security.py` gated `/mcp` and the console (`/d`, `/g`, `/h`, `/t`) on the same `MCP_BEARER_TOKEN`**, so sharing a dashboard link also handed over the ability to call all 49 tools — `w_add`, `w_remove`, `rg_journal_add`, `set_my_risk_profile` among them. Nothing about the console URL suggested it was an API credential.
- Added **`CONSOLE_TOKEN`**, optional and falling back to `MCP_BEARER_TOKEN`, so an unconfigured environment is byte-identical to before and the change cannot break a running deploy. Set, neither token opens the other's surface. Verified through the real ASGI app: API token on `/mcp` 200, console token on `/mcp` **401**, API token on `/d` **404**, console token on `/d` 200.
- The 401-not-404 on `/mcp` is because the path falls through to the bare-`/mcp` OAuth mount rather than the URL-secret one. Denied either way, but worth knowing so it isn't read as a routing bug.
- The full suite caught a consequence I had not anticipated: `test_route_returns_200_not_503` patched `MCP_BEARER_TOKEN` and the console routes no longer read it. That is the split doing its job; the test now patches `CONSOLE_TOKEN`.
- **Cloudflare Access is the chosen login**, unblocked because [niko] owns `tecxmate.com` on Cloudflare (Access needs a zone you control, so `*.zeabur.app` was never an option). Design decision worth stating: the console gets its **own hostname** rather than a path policy on the existing one — MCP clients and the Telegram webhook are not browsers and break the moment Access challenges them. Runbook on [console-auth](topics/console-auth.md).
- Deferred deliberately: verifying `Cf-Access-Jwt-Assertion` to drop the token from the URL entirely. It needs a JWT library in `mcp_server/requirements.txt`, and everything there ships in the deployed image — its own piece of work, not a rider on this one.

## [2026-08-16] feat | Plan tiers — show-and-refuse entitlement over the 49 tools
attributed_to: [niko, claude-agent]   belongs_to: [commercial-productization, mcp-server]
- [niko]: "apikey / account levels with tiered access to basic to premium tools… you think and decide for me." Most of it already existed — `customers.plan` was stored, returned and settable, and **never enforced anywhere**; `status` gave 402 and `monthly_quota` gave 429, both per HTTP request. The gap was that nothing mapped a tool to a tier.
- **Why the gap existed:** `_customer_gate` runs per HTTP request, but MCP is one endpoint carrying JSON-RPC, so the gate cannot see *which* tool is being called. Entitlement had nowhere natural to live.
- **Enforcement is one post-registration pass over the FastMCP registry**, not a decorator on 49 handlers. A decorator you must remember is one someone will forget — precisely how `sc_capabilities` drifted to 33 of 48. Here omission is impossible: an unregistered tool is the only unguarded one, and `test_every_tool_is_wrapped` pins even that.
- **Show-and-refuse, not hide.** A locked tool stays listed and returns `_locked` naming the plan that unlocks it. Hiding would make the registry and `sc_capabilities` disagree — the same drift, other direction — and a refusal the model can read is upsell rather than a dead end. The message explicitly says *don't reconstruct the answer from other tools*, or a paid screen gets rebuilt out of free primitives one call at a time.
- **Policy in code, assignment in data.** `TOOL_TIERS` lives in `tiers.py` because entitlement is security-relevant: a DB typo would open a paid tool with no diff, no review, no CI. Which plan a customer is *on* stays `customers.plan`, editable per customer without a deploy.
- **Two deliberately opposite failure directions:** unknown plan fails *closed* on entitlement (a typo must not hand out the paid surface) and *open* on quota (a wrong 0 would 429-storm a paying customer). Unknown tools default to PRO — withheld, not given away.
- **Backwards compatibility was the constraint that shaped the tiers.** Every existing customer was provisioned `plan=private` (the `provision_customer.py` default), so `private` means full access and its quota default is `None` — because NULL already meant unlimited and [niko]'s own row relies on it. Shipping this cannot downgrade or cap anyone who exists today.
- Split: 24 free (orientation, identity, single-name lookup — the whole documented `ticker_lookup → beginner_stock_card → q_valuation` path, pinned by a test), 25 pro (market-wide screens, the `q_*` quant suite, Risk Guard, digests). See [console-auth](topics/console-auth.md) for the sibling console work. 19 new tests; suite 589 pass.

## [2026-08-16] feat | Realtime layer, phase 1 — category switches, intraday stop watcher, macro news
attributed_to: [niko, claude-agent]   belongs_to: [risk-guard, system-architecture]
[niko] asked for real-time notifications and scheduled macro tracking. Phase 1, three pieces:
- **Telegram category switches** (`TELEGRAM_BRIEFS/ALERTS/NEWS/OPS`, all default on under the master). The first real use of `TELEGRAM_ENABLED=false` was about noise, and total silence also cost the stop alerts and morning brief — the messages the noise was drowning. One switch per kind of message. Unknown categories fail **open**: this gate reduces noise, and a typo at a send site must degrade to "delivered", never to a new silent channel. `send()`'s default category is `alerts` for the same reason.
- **Intraday stop watcher** (`src/quote_watch.py`) — the one signal that genuinely exists intraday: T86 flow is structurally end-of-day, but *price against a stop line* is not, and a stop breached at 09:47 used to be discovered at 15:00. Rides the news poller's 180s loop rather than being a fourth Zeabur service; polls Fugle (thin last-price client — the full client stays server-side with its `limit_board` dependency) for active positions during 09:00–13:35 Taipei, and reuses the **pure** `mcp_server.api.rg.stops.evaluate` for breach semantics — a third copy of stop logic is how the quant mirrors happened. Alerts carry the honesty line: 盤中價,不是收盤判定 — the post-close pipeline stays the verdict; kinds are `intraday_*` so both fire. Dedupe via `rg_alerts` write-first contract: one line, one day, one buzz, with pipeline's recorded-but-unpushed resend rule honoured.
...
## [2026-09-01] feat | Macro data layer — the first non-Taiwan series, and what a 7-agent scout found on the way
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, mcp-server, commercial-productization]
- Phase 2 of the realtime plan: `raw_macro` (sql/026) + `src/harvester/macro.py` + `q_macro` (tool 50) + a macro line in the pre-market Telegram brief. Five series — SOX, TSMC ADR, US 10Y, DXY, USD/TWD — the only data in this system **already known before the Taipei open**; everything else here is Taiwan-domestic and T+1.
- **Two vendors on purpose.** Yahoo v8 chart carries all five, but `us10y` is split to FRED: FRED is the official publisher, needs no key, and the split means a Yahoo outage costs four series rather than five.
- **Dates are US-session dates in UTC, not Taipei.** A US close stamped with a Taipei date lands a day ahead and silently misaligns every join against a TWSE date. The tool docstring, the glossary and a test all say so, because this is the kind of thing that is discovered six months later in a wrong correlation.
- **`raw_macro` must never become a second trading calendar.** `raw_twse_index` is the authoritative "did Taiwan trade" oracle (`loader.margin_sessions_missing`, `riskguard.store.last_trading_day`, pinned by `tests/test_margin_catchup.py`). Macro rows exist for US sessions on days the TWSE was shut — that is the feature, and also the trap.
- **Grant ordering is load-bearing:** 026 is DDL-only and its `mcp_viewer` grant lives in 024, per the two silent traps CLAUDE.md documents. In `apply_delta.py` 026 is sequenced **before** 024, because 024's grant loop skips tables that do not yet exist — and `raw_macro` was added to `READ_TABLES` so the privilege read-back fails the run if the grant did not land.
- **`raw_macro` added to `db_v2.query_data_status`'s hardcoded table list** — that list is what both `sc_data_status` and the operator console overview render, so without it a macro harvest that silently stopped would be invisible on the exact page you would open to check.
- **The container cannot test any of this end to end.** Egress here is a GitHub/PyPI allowlist that blocks Yahoo, FRED *and* `www.twse.com.tw` — the repo's own production sources — so a 403 here proves nothing about production. The parsers are therefore pure functions over captured payloads and fully tested offline; the fetch path is thin, and is unverified until it runs on Actions.
- **Found by the scout, worth its own line:** `src/harvester/daily.py::main()` discards `harvest_today()`'s return and never exits non-zero, so ANY step failing produces a green Actions run and no failure alert. Not changed here — failure isolation is deliberate and flipping it is an ops decision — but it means the macro step's own `ingestion_log` row (`source='macro'`, status `ok`/`partial`/`error`) is the only signal that it worked. Raised for [niko].
- Also fixed: the 07:30 pre-market Claude Routine had **no trading-day guard** and would have briefed on Taiwan holidays; Brian's pre-existing 08:30 "Taiwan Daily Brief" has one. Guard added, and the two routines' scopes made explicit (his = personal book, calm/hold-by-default; niko's = supply chain + theses).
- 19 new tests; suite 632 pass, ruff clean, 50 registered = 50 advertised = 50 tiered.

## [2026-09-01] fix | The nightly harvest can finally fail — without amputating Risk Guard
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, risk-guard]
- `src/harvester/daily.py::main()` discarded `harvest_today()`'s return and had no `sys.exit`, so `python -m src.harvester.daily` exited **0 no matter how many steps raised**. A harvest could fail every night behind a green Actions run, a green `hard` in the cron chain, and no `if: failure()` alert. Found by the macro-layer scout; [niko] chose "exit non-zero if any step failed" over silence.
- **The naive fix would have been strictly worse than the bug.** In `daily_harvest.yml` the harvest step had no `continue-on-error`, and the step immediately after it is the Risk Guard post-close pipeline — the one that file already marks *"NOT continue-on-error: a silent failure there is a stop-loss alert that never fired"*. A failing harvest step stops the job, so a flaky TWSE fetch would have skipped the stop-loss run. `deploy/daily-chain.sh` had the same shape: `hard python -m src.harvester.daily` aborts the chain before `riskguard.pipeline`.
- **Resolution — defer the failure past the work.** The harvest step absorbs its own failure (`continue-on-error: true`, `id: harvest`), every downstream step runs on whatever data DID land, and a **`Harvest verdict`** step at the end re-raises it before `Notify on failure`. The run still goes red and still alerts; nothing is skipped to achieve that. The verdict reads `steps.harvest.outcome`, **not** `conclusion` — `continue-on-error` rewrites `conclusion` to `success`, which is exactly how this indirection would silently stop working.
- On Zeabur the harvest drops to `soft` (that service carries no `TELEGRAM_TOKEN` by design, so Actions is the reporting path and the chain's only job is to keep running); `riskguard.pipeline` stays `hard` — its failure genuinely is fatal.
- Accepted cost, recorded so nobody "fixes" it later: one flaky fetch now reddens a run whose other steps succeeded and whose data is committed. **A red run does not mean no data landed** — `ingestion_log` / `sc_data_status` carry the per-step truth.
- 10 new tests, including a mutation-verified guard that the harvest step stays `continue-on-error`, that the verdict sits after Risk Guard and before the notify step, and that the cron chain does not abort ahead of Risk Guard. Suite 642 pass.

## [2026-09-01] feat | Two investor characters, and the arithmetic that makes the risky one defensible
attributed_to: [niko, claude-agent]   belongs_to: [investing-principles, mcp-server, commercial-productization]
- [niko]: "one is a conservative long term investor one is a opportunist who can trade to make money. i know it's more risky. bake that in and **estimate the risk well**." That last clause is the design: an aggressive persona without numbers is a personality, and a reckless one — "be aggressive" with no sizing math is just encouragement.
- **The personas ARE the risk profiles, not a new system.** `customers.risk_profile` (conservative|balanced|aggressive, sql/023) already existed and `my_profile` already returned `how_to_adapt`. `conservative` = **The Steward**, `aggressive` = **The Opportunist**, `balanced` = The Allocator. `index._RISK_GUIDANCE` is now GENERATED from `personas.py` rather than hand-copied — a second list of the same three things is exactly what `sc_capabilities` did when it drifted to 33 of 48, and a test pins the generation.
- **`position_risk.py`** — pure, no DB/clock/network, same rationale as `rg/`: a risk number you cannot re-derive from stated inputs is one nobody should size money on. Computes ATR-based stop, position size against the user's own risk budget, exit liquidity, and the Taiwan-specific piece generic models miss: **±10% daily limit means a stop does not fill in a limit-down**, and a stop further than one full limit is gapped straight through. Exposed as `risk_estimate` (PRO); `investing_personas` (FREE) explains the characters.
- **Sizing rounds lots DOWN**, deliberately — rounding up silently exceeds the one number the caller asked to control. Mutation-verified: switching to `ceil` fails two tests.
- **`estimate()` degrades by section and NAMES what it could not compute.** No volume → "liquidity is UNKNOWN, not fine". No stop and no ATR → it refuses to size anything at all. Silence about an uncomputed risk reads as no risk, which is the failure this module exists to prevent.
- **The non-advice boundary is unchanged and now enforced per-persona**, not left to the server `instructions` alone — the aggressive persona is the one a determined user pushes on. Every persona's `how_to_adapt` carries "never tell the user to buy, sell or hold", and a parametrised test asserts it for all three; stripping the sentence fails three tests. The Opportunist additionally `refuses` to imply a probability of profit: these numbers bound the LOSS, not the odds.
...
## [2026-09-01] feat | Macro goes global — Japan, Korea, China, Europe, and the timing distinction they forced
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, mcp-server]
- [niko]: "when I mean macroeconomics, I think you should also check Japan, Korean, Chinese, US, Europe market." Six series added — `nasdaq` ^IXIC, `estoxx50` ^STOXX50E, `nikkei` ^N225, `kospi` ^KS11, `shanghai` 000001.SS, `hangseng` ^HSI — taking `raw_macro` from 5 series to 11. No migration: the table keys on `(date, series)` and `series` is free text, so new markets are new rows.
- **The addition broke the module's stated premise, and that is the real content of this change.** `src/harvester/macro.py` opened with "these five series are the only data ALREADY KNOWN before the Taipei open" — true of SOX/TSM/10Y/DXY/TWD. **Tokyo, Seoul, Shanghai and Hong Kong trade at the same time as Taipei.** Their stored row is a PREVIOUS close while today's move is still happening, so reciting a live KOSPI as "overnight macro" is a false statement about the world, not a formatting slip. Every series now carries `when_known` ∈ {`before_open`, `same_session`}; `q_macro` returns it per row and glosses it; the `_freshness` stamp stopped saying "known before the Taipei open" because that string was about to become a lie on 4 of 11 rows.
- **The pre-market brief emits TWO lines, not one.** 隔夜 Macro (US + Europe, genuinely overnight and able to inform the open) and 亞洲鄰近市場 (…前一日收盤,尚未開盤). At 08:30 Taipei the Asian peers' newest close is exactly as old as Taiwan's own — worth reading, but not as news about today. One line would have put two ages of information under one honest-looking label.
- **Why these indices.** Korea is the closest comparable and the one usually left out: Samsung and SK Hynix sell into the same memory cycle as Taiwan, so a KOSPI/TAIEX divergence is signal rather than noise. Japan carries the semicap complex (Tokyo Electron, Advantest). Shanghai is mainland demand and Hang Seng is where China tech is actually priced — different animals, so both rather than one as a proxy. Europe earns its place on timing alone: it closes ~00:30 Taipei, the last read before the open.
- **`SERIES_META` is now the single source of truth** — symbol, vendor, market, label, `when_known` — and `YAHOO_SERIES`/`FRED_SERIES`/`ALL_SERIES`/`MARKETS` are all derived from it. Four parallel maps is exactly how `sc_capabilities` drifted to 33 of 48.
- **`index.py` mirrors that metadata because it cannot import `src/`** (Docker build context is `mcp_server/`, same constraint as the mirrored `quant/` trees). A test pins market AND `when_known` for every series in both directions: a series the server calls `before_open` while the harvester calls it `same_session` is worse than a missing one — it is confidently wrong.
- **`_utc_date`'s invariant is now written down.** Yahoo stamps a daily bar at the session OPEN, so a UTC date is the market's own session date only while that open falls on the same UTC day. It holds for all eleven (Tokyo opens 00:00 UTC, Hong Kong 01:30, Frankfurt 08:00, New York 14:30) but it is a property of these exchanges, not of the code — the ASX (10:00 AEDT = 23:00 UTC the previous day) would be stamped a day EARLY. Tested, including the ASX near-miss.
- Yahoo calls went 4 → 10 in one change, so `fetch_series` now paces them (`MACRO_REQUEST_DELAY`, default 0.5s). Four unspaced calls never drew a 429; ten is a different ask of a free endpoint and a 429 costs a whole series for the day.
- Also corrected: the `sc_capabilities` entry for `q_macro` still read "Global macro set before the Taipei open: SOX, TSMC ADR, US 10Y, DXY, USD/TWD" — stale on contents and repeating the same false timing claim.
- 33 new tests; suite 709 pass, ruff clean. Mutation-verified three ways: labelling KOSPI `before_open` fails 6 tests, putting every series on the brief's 隔夜 line fails 2, and desynchronising the server mirror from the harvester fails 2.

## [2026-09-01] feat+fix | Quant-fund strategies catalogued honestly — and the backtester that had to be fixed first
attributed_to: [niko, claude-agent]   belongs_to: [mcp-server, investing-principles, commercial-productization]
- [niko] asked what could be mimicked from Renaissance, Citadel, Millennium, Two Sigma, AQR, Man AHL and the Chinese quant houses (High-Flyer/幻方). **The organising answer: what transfers from quant investing is the RIGOR, not the signals.** The published premia (factors, trend following) work on daily closes; Medallion's edge is inseparable from tick data, co-located execution and a closed capital base, and Citadel Securities' edge is *being* the market maker. Neither is a strategy that can be re-run.
- **That answer was unusable until `q_backtest` was fixed**, so the deferred audit (flagged when the personas shipped) happened here. Six defects, **all biasing the same way — optimistic**:
  1. **No baseline.** A 58% hit rate is nothing if 56% of all bars rose over the same horizon. This turned "the market went up" into "my signal works" and was the single most misleading omission.
  2. **No transaction costs.** Taiwan round trip ≈ 0.585% (0.1425% brokerage ×2 + 0.30% sell-side transaction tax). A 5-day rule averaging +0.4% gross is a LOSING rule that the old output reported as a winner.
  3. **Look-ahead entry.** Return was measured `close_{t+N} / close_t` — buying at the very close the signal was *computed from*. RSI_14 on date t is a function of that close; you cannot see it, decide, and transact at it. Default entry is now `next_close`, and `same_close` is kept only so the difference MEASURES the look-ahead.
  4. **Overlapping windows inflated n** — a name triggering six days running gave six observations with near-identical forward windows.
  5. **Cross-sectional correlation inflated n further** — 200 Taiwan semis triggering on one afternoon is ~1 observation, not 200. Clustering by DATE fixes 4 and 5 at once and is the conservative choice.
  6. **No interval.** 40 effective observations were presented as confidently as 4,000. Now a Wilson interval on the EFFECTIVE n — Wilson because the normal approximation is worst exactly where the tool is most likely to be believed (small n, extreme proportions).
- `quant/evidence.py` is pure (mirrored to `src/quant/`), and its `verdict` checks sample size FIRST: a spectacular edge on 9 independent observations is not a weak result, it is an absent one. **What it cannot fix, it names**: survivorship is uncorrected and upward-biased, because the universe is the one classified TODAY applied backwards and this repo records no point-in-time membership. In-sample tuning and excluded slippage are named too.
- `systematic_strategies` (tool 53, FREE) catalogues nine strategies with a `status` of available / buildable / **out_of_reach**. The `out_of_reach` entries are the point — HFT market making and ML alpha ensembles are refused with reasons (exchange position; sample size, not compute), and a test asserts they list **no tools**, because a moving-average crossover wearing a famous fund's name is a worse answer than "that cannot be done here". High-Flyer's return compression under quant crowding is recorded as a capacity lesson, not a modelling one.
- **The most transferable idea on the list is the least glamorous:** Millennium/Citadel pod risk discipline — mechanical, pre-committed loss limits nobody may argue with in the moment. That is Risk Guard, already built. Marked `available`, and a test pins it.
- `QUANT_PRINCIPLES` is kept SEPARATE from `investing_principles` so the latter stays what it claims to be — school-neutral universals. Each quant principle names the tool that enforces it, and "capacity decays edge" honestly says **nothing** enforces it; a test pins that admission.
- 42 new tests; suite 742 pass, ruff clean, 53 registered = 53 advertised = 53 tiered. Mutation-verified four ways: dropping cost subtraction fails 2, using raw n for the interval fails 1, measuring edge against zero fails 1, reverting the entry default to `same_close` fails 1.
- Note: `src/quant/backtest.py` is an older standalone CLI nothing imports and it still carries the same-close look-ahead. Left alone deliberately — `db_v2.query_backtest` is the live path — but it is a trap for whoever runs it.

## [2026-09-01] docs | A tutorial that cannot go stale, because CI rebuilds and compares it
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, mcp-server]
- [niko]: "Think we need to write a tutorial on how to use the system now that it has gone quite big and comprehensive. **Tutorial also need to be kept up-to-date.**" That second sentence is the design constraint, not a nice-to-have: this system gained 5 tools in one day, and a hand-written guide would have been wrong by evening. **A confidently wrong instruction costs more than a missing one.**
- `docs/TUTORIAL.md` is GENERATED by `scripts/build_tutorial.py`. Everything that can drift is read from the running code rather than retyped: tool names and one-line purposes from `sc_capabilities()` (which `tests/test_capabilities.py` already pins to the FastMCP registry, so the tutorial inherits that guarantee), plans from `tiers.TOOL_TIERS`, characters from `personas.PERSONAS`, the catalogue from `strategies.STRATEGIES`, and the series table from `macro.SERIES_META`.
- **The tripwire is `tests/test_tutorial.py`**, which re-renders and fails if the committed file differs — the same mechanism that stopped `sc_capabilities` drifting to 33 of 48. `--check` gives a contributor the same signal from the command line, so the CI failure is actionable without reading the test.
- **The assertion that actually earns its keep** is not the byte-compare, it is that every registered tool lands in exactly ONE section of `GROUPS`. A new tool can otherwise be correctly registered, correctly tiered and correctly advertised — and still invisible to any human reading the docs. Mutation-verified: adding a tool without touching the tutorial fails 2 tests naming it; hand-editing the file fails 4.
- Narrative prose lives in the generator, not a separate template — one source, no merge step. Edit the script, run it, commit both.
- Content decisions worth keeping: the three-surface split leads (Claude = analysis you reply to, Telegram = the machine's voice, console = pictures); `q_backtest` is taught **baseline-first** with an explicit "never quote a bare hit rate", because teaching the hit rate first teaches the mistake; the persona table renders `leads_with[0]` after the first render dumped a raw Python list repr into a table cell.
- Also corrected: `README.md` still advertised "48 read-only MCP tools" (now 53). Replaced with "50+" rather than a new exact number — a hardcoded count in an ungenerated file is the same rot, one release later.
- 13 new tests; suite 755 pass, ruff clean.

## [2026-09-05] fix | Three more nightly failures, found because the harvest finally reports honestly
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, risk-guard]
- [niko] ran **Actions → DB Migrate** and asked for a check. The migration is good: 026 applied **before** 024 (the ordering 024's grant loop depends on), `mcp_viewer SELECT verified on 8 backfilled tables` including `raw_macro`, and the live connector now answers `q_macro` with the full 11-series contract instead of `permission denied`. It returns `count: 0` because the harvester has not run since the table existed — empty, not broken.
- **Reading the harvest logs to confirm that turned up three unrelated failures.** Worth recording that this is the 2026-09-01 exit-code change paying for itself: the runs were red, so someone looked.
- **`thesis_status` has NEVER persisted a digest row.** `persist_digest` inserted into a column `inputs` (the schema calls it `source_inputs`) *and* omitted `title`, which is NOT NULL with no default — two independent errors in one statement. The step is `continue-on-error`, so every night the Telegram message went out, the INSERT raised `UndefinedColumn`, and the run stayed green. `src/cron/brief.py` writes the same table correctly; this one drifted from it with nothing comparing the two. `tests/test_digest_writers.py` now parses `sql/006_digests.sql` and pins **every** writer against it — unknown columns and missing NOT-NULL-without-default columns both fail. Mutation-verified: restoring either original bug fails 2 tests.
- **`kaleido==0.2.1` is incompatible with `plotly>=6`**, which raises "Image export requires the Kaleido package, v1.0.0 or greater". Worse than a missing image: the PNG write sat **between** `graph_snapshot.json` and `graph-view.html` in `correlation_snapshot.main()`, so it took the HTML viewer down with it — the JSON kept updating while the interactive 2D graph silently froze. Fixed both: pin bumped to `kaleido>=1.0.0`, and the PNG is now `write_png_best_effort()`, isolated so the next renderer problem costs only the image. kaleido v1 shells out to Chrome rather than bundling a renderer; verified here against the REAL failure (not a mock) by pointing `BROWSER_PATH` at the container's Chromium — render succeeds with it, degrades cleanly without it.
- **Branch protection has been rejecting the nightly dashboard push since 2026-08-16.** `GH013: Changes must be made through a pull request` on `chore(graph): daily refresh [skip ci]` — so the console's flow dashboard, ticker pages and graph view have been **stale for three weeks**. It hid because the step is `continue-on-error` and the runs were red for the macro reason anyway. **Once tonight's run goes green it would have become a genuinely silent failure**, which is the exact shape this pipeline keeps producing, so the step now emits a `::warning` naming the cause and the consequence ("the dashboards are STALE"), matching the Telegram-preflight idiom already in that file. Pinned by tests that fail if the annotation or the GH013 match is removed.
- **Open, and it is a repository-settings decision, not a code one:** the durable fix is to let the Actions actor bypass the ruleset for that one `[skip ci]` data commit. Alternatives are to have the job open a PR (a PR a day, for generated data) or to stop committing generated assets to `main` at all. Raised for [niko].
- 18 new tests; suite 773 pass, ruff clean.

## [2026-09-05] fix | One oversized news URL was discarding entire harvest runs
attributed_to: [niko, claude-agent]   belongs_to: [system-architecture, mcp-server]
- [niko]: "i think the run failed many times." Correct, and I had been looking at the wrong workflow — I diagnosed `daily_harvest.yml` and never checked the others. **`news_harvest.yml` failed 6 of its last 40 runs** (2026-09-02..04, none since).
- Root cause: `raw_news.url` is the PRIMARY KEY, so every URL becomes a btree index row, and Postgres refuses one over 2704 bytes — `index row size 2848 exceeds btree version 4 maximum 2704 for index "raw_news_pkey"`. A Google News redirect URL (base64 payload, not a readable path) crossed it.
- **The raise was not the damage.** `harvest()` wraps EVERY source in a single `atomic()` transaction, so the exception rolled back the whole run: every article from every feed, not just the offending row. A 12-source harvest threw away all twelve sources' articles because one gnews item had a long URL.
- **And it was not only the cron.** `src/news/watch.py` imports `harvest._upsert` rather than copying it (deliberate, per the near-realtime-poller decision), so the Zeabur worker crashed on the same articles every ~180s for as long as they stayed in the feed. One fix covers both; a test now asserts the poller still shares the functions rather than having grown a local copy.
- Fixed by **pre-filtering** over-long URLs (`MAX_URL_BYTES = 2000`, headroom under the 2704 cap), not by catching the exception. That distinction is load-bearing and is its own test: a failed statement poisons the surrounding transaction, so a try/except would leave every later row failing with "current transaction is aborted" — a partial fix that looks complete. Measured in BYTES, not characters: a percent-decoded CJK URL is ~3× more bytes than characters and Postgres counts bytes.
- Dropping an article is logged at WARNING with the source and the size. Silently discarding rows is how this class of bug hides in the first place.
- **Why it "stopped" on 2026-09-04 without anyone acting:** the offending articles simply aged out of the feeds. The bug was latent, not resolved, and would have returned with the next long Google News URL.
- Worth noting for the alerting posture: `news_harvest.yml` is NOT one of the three workflows that alert on failure (`daily_harvest`, `riskguard_premarket`, `db_backup`), so these six failures were visible only in the Actions log. With `TELEGRAM_ENABLED=false` they would have been invisible either way — [niko] found them by looking, which is the intended fallback but not a monitoring strategy.
- 12 new tests; suite 785 pass, ruff clean. Mutation-verified: removing the pre-filter fails 5 tests (including "every other article in the batch still lands"), measuring characters instead of bytes fails 1.
