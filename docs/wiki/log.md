...
- Found scheduled DB backups failing because `pg_dump` 17/16 cannot dump Zeabur Postgres 18.4; updated `db_backup.yml` to install `postgresql-client-18` and put its bin directory first on `PATH`.
